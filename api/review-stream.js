...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey) {
      sendSse(res, 'error', { message: 'ANTHROPIC_API_KEY is not configured.' })
      return
    }
    ...
    const sourceContent = await buildSourceContent(supabase, composition, assets)
...
async function loadSubmission(supabase, compositionId) {
  ...
  const { data: assets, error: assetsError } = await supabase
    ...
    .select('asset_type,byte_size,mime_type,original_filename,storage_bucket,storage_path')
  ...
  for (const asset of assets) {
    ...
    if (asset.asset_type === 'pdf') {
      if (asset.byte_size > MAX_PDF_BYTES) {
        throw new Error(`${asset.original_filename} is too large for direct PDF review.`)
      }
